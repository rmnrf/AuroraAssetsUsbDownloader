...
```text
┌────────────┐     ┌──────────────┐     ┌────────────┐
│   Xbox 360 │ ──→ │   USB Flash  │ ──→ │     PC     │
│  Content.db│     │              │     │ Downloading│
└────────────┘     │              │     │   covers   │
                   │              │ ←── │from Archive│
┌────────────┐     │  GameData/   │     │            │
│   Xbox 360 │ ←── │  *.asset     │     └────────────┘
...
### Step 1: Copy Content.db from Xbox

Copy the `Content.db` file from your Xbox to a USB drive:
```text
Xbox HDD: Aurora\Data\DataBases\Content.db → USB Flash Drive
```
